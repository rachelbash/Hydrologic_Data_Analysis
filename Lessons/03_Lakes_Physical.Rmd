...
=======
a
>>>>>>> faabfe312f632e51683e5e2a81d84200ddd4ff89
## Lesson Objectives
...
## Session Set Up
- if you want to go backwards in the file path, use .. instead of just one .
```{r, message = FALSE}
...
# Let's choose the three lakes with data, %in% means includes, | means OR
NTLdata <- NTLdata %>%
...
#Because it gives us the min, max, median, etc. We don't want that, just a list of depths
```
...
<Surface temps are higher at surface, but greater variability. At deep depths it is lower temp, with small variability. As lake depth increases, temp decreases.>
...
Here we are highlighting depths considered the **epilimnion** (closer to surface mixed layer) and **hypolimnion** (lower level). Differences in densities between bottom and middle resist mixing. 
```{r}
Tempplot.mixed <- 
...
Here are are highlighting depths considered the **metalimnion** or **thermocline**. Where there is a difference in temp, a rapid temp decline
```{r}
...
#to do a reverse graph, use scale_y_reverse and scale_x_continuous (position = "top")
#line represents 4 degrees Celcius: max density of water is at 4 degrees. Unless there is actual frozen water, 4 degrees is the coldest you will get
#surface of lake is getting warm from contact with atm. Light and energy doesn't penetrate down to bottom

#epilimnion surface mixed layer: 0-2 m
#2-5m is thermocline (diffusion)
#5m and lower hypolimnion 
...
#epilimnion has gotten warmer and warmer throughout the year, but still remains between 0-2m
#massive density/temperature gradients in thermocline, increases throughout the summer
```
<add notes here>
...
<data collection errors, it should look smooth but human error happens>
...
- dimictic: mixes twice throughout the year. Fall turnover (air cools, causing loss of heat from lake, lower density gradient) and happens in the spring after ice melt
...
wind driven, small enough density gradient where force of wind is able to mix up density differences within the water column
...
- epilimnion cuts off when there is a 1 degree temp change in less than a meter.