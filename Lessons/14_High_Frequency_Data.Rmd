...
# install.packages("StreamPULSE") these don't work
# install.packages("streamMetabolizer") not on CRAN, so do below instead:
...
#plotting concentration against flow (C-Q plots)
```

Which of these rivers is more chemostatic?

> Chemostatic: concentration of something in the river does not change with changes in discharge. Crane Creek is chemostatic because there is no relationship. Potomac river is NOT chemostatic because N concentration changes over volume of flow.
...
> Nitrate dec as flow inc (storm) - diluting the concentration with quick flow
> Flushing storm would be where concentration inc as flow inc
> Histeresis: behavior will be different at different times throughout the event. Due to sources of water coming from different pathways at the different times.
...
> Counter-Clockwise behavior where concentration decreases at the beginning of the storm (diluting) and then overtime increases concentration at the beginning of the storm. Slopes of the rising (quickflow) and falling limbs (baseflow) are different because they are coming from different sources that have different speeds at which they arrive at the river. Quickflow has lower concentrations of nitrate than in baseflow.

> Flushing system: increasing concentration from pushing the solute out of the system
...
> how fast ecosystem consumes and produces carbon

Why would we want to know the *GPP* and *ER* of a river reach?

> Gross Primary Productivity (GPP):produces oxygen and ER (Ecosystem Respiration): anything that produces CO2. Important to know how much C is in a stream - answers a lot of ecological questions. Can help classify stream productivity
...
```{r}
citation('streamMetabolizer') #uses oxygen data to create a metabolism
```

```{r}
Enodat <- StreamPULSE::request_data(
  sitecode = "NC_Eno",
  variables = c('DO_mgL'),
...
Eno.lon <- Enodat[[2]]$lon #double bracket is indexing and calling the item in that list. longitude from the second list

Eno.DO <- Enodat[[1]] %>%
  spread(value = value, key = variable) %>% #spreads out so instead of one column with all variables, it has multiple columns for each variable
  mutate(DateTime_Solar = convert_UTC_to_solartime(DateTime_UTC, Eno.lon)) #convert universal time to "solar time" where you just thought about the time when the sun is out, using longitude data.
...
> Productivity, respiration, and diffusion
> Productivity increases O concentrations only during the day, respiration happens all the time throughout the day
...
> Lots in US, few in Africa and Asia. Cannot interpret findings globally
...
> They just units of Oxygen instead of CO2 because they are often opposite and it is easier to measure.
> Kernal Density plot: draws boundaries where most of the points fall. 
> GPP is lower than ER, so the Eno it is a net consumer of Carbon. Net heterotrophic.
