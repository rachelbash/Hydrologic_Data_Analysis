...
> Differences in cost to obtain bacteria measurements. Some areas have more certain types of bacteria than others.

Where and when do you think the biggest risks from harmful bacteria are?

> After storms, low income communities, 
...
> near pollution sites and after WWTP, after storms
...
> Challenges: no one piece of data for one type of bacteria. All measured differently, can't easily compare across  
...
> Used to indicate whether other harmful bacteria may be present. If it is cheaper to collect data on this, may be better to do this regularly than to test for more expensive bacteria.
...
> Highest at end of summer - because lots of things are growing, farm has lots of animals
...
> Concord, because it is smaller. A storm would have a great effect on the discharge contents of the river

What would a CSO event look like in the data?

> A sharp peak shortly after the storm.
...
begin.date <- as.Date("1973-08-28", format="%Y-%m-%d")
end.date <- as.Date("1973-10-29", format="%Y-%m-%d")
  
discharge.plot <- ggplot(data =  discharge.dat, 
                 aes(x = Date, y = Discharge_cfs)) +
            geom_line() +
            scale_x_date(limits= c(begin.date, end.date) , name  = "") +
            lims(y=c(0,300))
