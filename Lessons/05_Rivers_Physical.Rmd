...
- temperature
- mixing (turbulent and laminar flow)
- slope
...
print(DvsGHplot)
```
> The shape of this curve is logarithmic. Empirically determined discharge
...
> lat, long, coordinate datum, altitude, HUC ID, 
...
* select function selects columns, filters selects rows
...
> Great difference in magnitude, Eno is largest magnitude, Sauk has overall lowest magnitude with lowest peak. 
...
> Bitteroot has seasonal, as does Sauk and Nahelem, though not as clearly as Bitteroot.Spring snow run-off is likely to have caused the seasonal cycles - because they are all in areas with snowpack.
...
> rbind (combine dataset on top of one another - must have same column titles for every df), Then label all discharge columns the same. Then graph discharge by date, facet by site number.
...
BitterrootDischarge.Historical <- mutate(BitterrootDischarge.Historical, 
                                         DOY = yday(Date)) #day of year from lubridate package

BitterrootDischarge.Pattern <- BitterrootDischarge.Historical %>%
  group_by(DOY) %>% #have all Jan 1 values and group them together
...
- vary by location and local weather patterns
