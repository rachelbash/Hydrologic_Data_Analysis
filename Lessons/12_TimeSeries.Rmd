...
**monotonic**: inc or dec consistent trend over time. For examples, responses of nutrients in response to land use change.
**step**: if policy were enacted, there would be a distinct shift at a given point
...
> aggregation: daily turned to weekly or monthly. Useful when you want to average values across a month

> subsampling: if you have daily data one year, but another year you only have weekly data. you might want to subsample the daily data so that it matches the other part of your data set.

> interpolation: missing data points where you can fill those in by averaging the points around the missing row, or using an average monthly value. fill in gaps.

Specific tests for monotonic trend analysis are listed below, with assumptions and tips: 

* **linear regression**: no seasonality, fits the assumptions of a parametric test. well approximated by a normal distrubtion. Function: `lm`
* **Mann-Kendall**: assumes no seasonality, non-parametric, no temporal autocorrelation, missing data allowed. Function: `mk.test` (package: trend)
* **modified Mann-Kendall**: no seasonality, non-parametric, *accounts for temporal autocorrelation*, missing data allowed. Function: `mmky` and `mmkh` (package: modifiedmk). incorporate autocorrelation into model.
* **Seasonal Mann-Kendall**: seasonality, non-parametric, no temporal autocorelation, identical distribution. Function: `smk.test` (package: trend). needs exact frequency between every data point (e.g. every 14 days)
...
Mendotadata <- LAGOSnutrient %>%
  ...
  select(sampledate, tp) #dates are not stored chronologically, must deal with eventually
...
* **Linear**: average the points around it to get the mean
* **Spline**: piecewise polynomial function. more robust. draw smooth curves across data points
...
linearinterpolation <- as.data.frame(approx(Mendotadata, n = 303, method = "linear")) #how many months do you want it to generate? n=303 months from july 1988 thru aug 2013
#approx gives us interpolation
#x used to be dates, now its some strange number (number of days since Jan 1 of 1970)
...
#new interpolated points are in pink. they will never go above or below the actual data, which is good
...
#frequency is 12 months (12 seasons), with start in July, 5th
#you don't have to throw data away if you are starting in the middle of the year

# Run SMK test
Mendotatrend <- smk.test(Mendotatimeseries)

# Inspect results: two different way
Mendotatrend #overall trend. p > 0.05 not significant, no overall trend upward or downward in the data
#z score gives us direction and magnitude of significance. So its negative, but its magnitude is not high enough
#S counts up total magnitude of shift over time. counts up difference between points
summary(Mendotatrend)
#tests separately by month
#october and nov have significant p values, with neg trend in both

```

What would we conclude based on these findings? Describe as you would to an educated but non-specialist audience.

> Overall, there is no significant positive or negative trend in tp over the 34 years, however there is a significant negative trend in tp in oct and nov.
...
ClearCreekDischarge.Monthly <- ClearCreekDischarge %>%
  mutate(Year = year(Date), 
         Month = month(Date)) %>%
  group_by(Year, Month) %>% #group by both b/c you want one value for each individual month across that dataset
  summarise(Discharge = mean(Discharge))
    
ClearCreek_ts <- ts(ClearCreekDischarge.Monthly[[3]], frequency = 12) #column 3 has a frequency of 12
...
```{r}
adf.test(ClearCreek_ts, alternative = "stationary") #our p-value < 0.05 so data is stationary
```
...
#AIC characterizes how well the model fits the data
#trace=true shows which models it is testing
...
>could use co-variance to improve forecasting 
