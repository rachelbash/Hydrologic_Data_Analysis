...
* **Autocorrelation**: Data points are not independent from one another (i.e., the measurement at a given time point is dependent on previous time point(s))
  - you can account for this in time series analysis
...
#difference between cyclical and seasonal - cyclical can be supraseasonal time scales
#very clear variance by season
...
#not very effective for this option
#works better: something that eliminates seasonality
#decompose data into annual means - then you are able to use linear regression
```
...
```{r}
ClearCreek_ts <- ts(ClearCreekDischarge[[4]], frequency = 365) #can only provide one column of data
#important about time series: there must be regularly spaced data! Step to each row must be the same each time.
#if NA's, you would have to interpolation to fill in those gaps and maintain the equal steps between each row
#if you want to see if there is a difference between days so you can tell whether you are missing dates, use this:
table(diff(ClearCreekDischarge$Date))
#this means that one is missing!
...
```{r}
?stl #take moving average along window
# Generate the decomposition
ClearCreek_Decomposed <- stl(ClearCreek_ts, s.window = "periodic")

# Visualize the decomposed series. 
plot(ClearCreek_Decomposed)
#trend + seasonal + remainder = our data
#trend is the moving average trend
#remainder is all of the error that is not seasonal or trend related
...
ggplot(ClearCreek_Components) +
  ...
  geom_hline(yintercept = 0, lty = 2) +
  ylab(expression("Discharge (ft"^3*"/s)"))
#there are negative numbers, math is agnostic, doesn't make sense, but just accept that it is doing it correctly
```
