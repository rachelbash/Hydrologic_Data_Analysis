...
```{r}
EnoDischarge <- mutate(EnoDischarge, metric_Discharge = Discharge*0.0283168)
...
```{r}
EnoDischarge <- mutate(EnoDischarge, rolling_mean = rollmean(x=metric_Discharge,k=30, fill = NA))
...
```{r}  
ggplot(EnoDischarge) +
    geom_line(aes(x = Date, y = metric_Discharge, color="Daily"), color="darkgrey") +
    geom_line(aes(x=Date, y= rolling_mean, color="30-day"), color='green') +
    labs(x="", y=expression("Discharge (m"^3*"/s)")) +
    geom_label_repel(data = subset(EnoDischarge, Date == "2018-09-17"),
            aes(x=Date, y = metric_Discharge, label = "Hurricane\nFlorence"),
            color = "darkgrey", segment.color = "darkgrey", 
            min.segment.length = 0, nudge_x = -1000, nudge_y = 10000, size = 3
            ) +
  Rachel_theme +
