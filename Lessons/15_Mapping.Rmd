...
* **Point** <has x and y components, two data values>
* **Line** <connection of points, sf function would hold multiple values of the line into one cell>
* **Polygon** <connection of lines that end at the beginning to make one shape>
...
LAGOScombined <- 
  left_join(LAGOSnutrient, LAGOSlocus) %>% #didn't have to specify which columns to join by if they have the same name
...
secchi.summary <- LAGOScombined %>%
  ...
  summarise(secchi.mean = mean(secchi),
            ...
            depth = mean(maxdepth),
            lat = mean(nhd_lat), 
            long = mean(nhd_long)) %>% #the mean won't be any different from the individual observations, we just need to do this in order to include it in the new table we are creating
  drop_na()
```
...
#crs is the projection id number, giving it both long and latitude columns, sometimes you will just provide the geometry column for coords
...
> It seems like the bigger the lake (area), the more likely you will find a shallow secchi depth. And there are more medium depth lakes that have deep secchi depths.

```{r}
model1 <- lm(data=MEsecchi.summary, secchi.mean ~ area + depth)
summary(model1)
```
