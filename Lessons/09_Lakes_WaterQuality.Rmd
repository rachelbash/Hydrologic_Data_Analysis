...
- DO
- nutrients - eutrophication (Nitrogen and Phosphorus)
- microbes
- odor/color
- invasive species (that affect food web)
- contaminants (heavy metals from industry)
...
options(scipen = 100) #anything up to the hundreth decimals places, it will replace that. Fixes issues with ggplot when it deals with scientific notation

lagosne_get(dest_folder = LAGOSNE:::lagos_path(), overwrite = TRUE) #install the dataset associated with lagos package onto computer. You only need to do this once, just to download onto local drive. Once it is done, then you can load data directly
...
LAGOSlocus <- LAGOSdata$locus #metadata about all lakes in the database, one row for every lake
LAGOSstate <- LAGOSdata$state #metadata about the state
LAGOSnutrient <- LAGOSdata$epi_nutr #don't have any depth related info, they just have surface water data
...
```{r}
# Join data frames
LAGOSlocations <- left_join(LAGOSlocus, LAGOSstate, by = "state_zoneid") #left join. Take everything from the left and join the df on the right, by joining them by the column that they both have in common
...
Trophic state can change over time- an indicator of larger problems. Also an indicator for other water quality problems.
...
* *Secchi disk transparency*, a measure of water clarity. Pros: simple and cheap. Cons: may yield a high TSI in highly colored lakes and in lakes where particulate matter is comprised of non-algal material (like dissolved organic matter and sediment)
* *Total phosphorus (TP)*, a nutrient essential for growth of primary producers. Assumptions: phosphorus is the limiting nutrient for phytoplankton growth (this assumption often holds only for summer months) (when P is high, lots of nutrient for primary production)
...
LAGOStrophic <- 
  ...
  mutate(sampleyear = year(sampledate), 
         ...
         season = as.factor(quarter(sampledate, fiscal_start = 12))) %>% #divide year into quarters, start in month 12 so first season is dec, jan, feb. Second quarter will be march, apr, may
  drop_na(chla:secchi) #chla, tp, and secchi. If NA in any of those rows, remove entire row
...
ggplot(LAGOStrophic, 
  ...
  scale_color_viridis_c(option = "magma")
print(secchibymonth)
#secchi depth has an inverse relationship to cholorphylla
...
> Phosphate is dissolved - readily available. Particulate phosphorus is not readily available, but both are being measured. Phosphorus increases in the summer - chlorophyll needs P in summer. More sampling points in the summer. 
...
#adding columns by using equation for TSI for each 
#log means natural log (log10 is real log)
#chain of ifelse statements
#ideally all TSI numbers should be the same regardless of index you used, but there is just naturally variability between the three
...
#geom_rect visualizes the different trophic states

```
What insights do we gain from the different visualizations?

> Violin: good for range

> Bar: good for counts, and separates into trophic state

> Jitter:can see seasonality and when they sample
