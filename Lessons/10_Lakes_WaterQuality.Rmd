...
> Variable 1: state (locus)

> Variable 2: secchi disk transparency (epi_nutr)

> Variable 3: doc (epi_nutr)

> Variable 4: meandepth (lakes_limno)
...
LAGOSstate <- LAGOSdata$state %>% select(state_zoneid, state_name)
LAGOSlocus <- LAGOSdata$locus %>% select(lagoslakeid, state_zoneid, gnis_name)
LAGOSepinutr <- LAGOSdata$epi_nutr %>% select(lagoslakeid, sampledate, chla, doc,tp,tn, secchi)
LAGOSdepth <- LAGOSdata$lakes_limno %>% select(lagoslakeid, meandepth)

LAGOSstatewrangled <-
  left_join(LAGOSstate, LAGOSlocus, by = "state_zoneid")

LAGOSskinny <-  
  left_join(LAGOSepinutr, LAGOSstatewrangled, by = "lagoslakeid") 

LAGOSwrangled <- 
  left_join(LAGOSskinny, LAGOSdepth, by = "lagoslakeid") %>% 
...
LAGOSwrangled.corr <- cor(LAGOSwrangled.final)
corrplot(LAGOSwrangled.corr, upper = "ellipse")
```

What patterns do you see?

> tp and tn are moderately positively correlated with chla. tn is more positively correlated with doc than tp is. DOC and mean depth are strongly negatively correlated with each other, meaning that as DOC goes up, mean depth goes down. 

2. Create two linear regression models (hint: function `lm`) to predict TN and TP using your five predictor variables. 

```{r}
model1 <- lm(data=LAGOSwrangled, tn ~ chla + doc + secchi + meandepth + state_name)
summary(model1)

model2 <- lm(data=LAGOSwrangled, tp ~ chla + doc + secchi + meandepth + state_name)
summary(model2)
...
> tn: only .3762 adjusted R2 value, not great.
