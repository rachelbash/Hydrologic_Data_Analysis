...
#creating a model that gives you a prediction line - can extrapolate
Eno.RImodel <- lm(data = EnoRecurrence, PeakDischarge ~ log(RecurrenceInterval))
summary(Eno.RImodel)
#discharge = 1332 + 1725*log(x)
#y = b + m*x
...
#full period of record has higher discharge events, with steeper slopes, esp at the beginning
...
> Fuller period of record is more complete --> more accurate, also shows that there were years with higher peak discharge
...
> Using the longest record that you have for your stream, or for discharges for rivers in the same area...is there a trend over time? Do we need to over estimate as we predict into the future. OR a moving average to use the more recent set of data over time.

> do we maximize the amoutn of data or do we choose
...
#models
Salado.RImodel.Early <- lm(data = SaladoRecurrence.Early, PeakDischarge ~ log(RecurrenceInterval))
summary(Salado.RImodel.Early)
Salado.RImodel.Late <- lm(data = SaladoRecurrence.Late, PeakDischarge ~ log(RecurrenceInterval))
summary(Salado.RImodel.Late)
Salado.RImodel.Early$coefficients
Salado.RImodel.Late$coefficients

#plots
SaladoRecurrencePlot.Full <- 
  ggplot(SaladoRecurrence.Early, aes(x = RecurrenceInterval, y = PeakDischarge)) +
  geom_point() +
  geom_point(data = SaladoRecurrence.Late, color = "#02818a",
             aes(x = RecurrenceInterval, y = PeakDischarge))
print(SaladoRecurrencePlot.Full)

#100 and 500 year recurrence intervals
Salado.RImodel.Early$coefficients[1] +Salado.RImodel.Early$coefficients[2]*log(100)
Salado.RImodel.Early$coefficients[1] +Salado.RImodel.Early$coefficients[2]*log(500)
